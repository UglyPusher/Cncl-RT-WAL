...
В автоматическом режиме : 
	...
	Читает сигналы "Пауза", "Стоп".  Видимо взаимодействует с интерфесной задачей ввода и интерфейной задачей вывода.  
...
________________________________________________________________________________________________________________________________________________________________
|RT_TIME buffer 		|IN_C_TIME   |TEMP_SNSR_TSK |NON_RT         |OUT_TEMP           |CRRNT_TEMP     | current temperature, tick 
...
________________________________________________________________________________________________________________________________________________________________
|RCP_DATA SPMC          |IN_RCP_DATA |HEAT_ACT_TSK  |RT/HAL-dep     |OUT_HTR_STATE      |HTR_STATE      | Heater on/off, tick
|CRRNT_TEMP             |IN_CRNT_TEMP|  -//-//-     |               |                   |               |
HEAT_ACT_TSK — включает/выключает нагреватель по гистерезису, читая уставку и текущую температуру.

RT. Период вызова критичен для своевременного переключения нагрева.
...
________________________________________________________________________________________________________________________________________________________________
|RCP_DATA SPMC          |IN_RCP_DATA |PUMP_ACT_TSK  |RT?????        |OUT_PMP_STATE      |PMP_STATE      | Pump state (on/off/rest), tick
|CRRNT_TEMP             |IN_CRNT_TEMP|  -//-//-     |               |                   |               |
...
Требование к HAL:
    ...
    не на задаче.
________________________________________________________________________________________________________________________________________________________________
|-------------          |----------- |FLOW_SNSR_TSK |NON_RT         |OUT_FLOW           |CRRNT_FLOW     | current flowmetr, tick 
...
________________________________________________________________________________________________________________________________________________________________
|—                      |—              |TEMP_SNSR_TSK |NON_RT         |OUT_TEMP           |CRRNT_TEMP SPMC |current_temp, tick
|RT_TIME buffer         |IN_C_TIME      |-//-//-//     |               |                   |                |
________________________________________________________________________________________________________________________________________________________________
|—                      |—              |FLOW_SNSR_TSK |NON_RT         |OUT_FLOW           |CRRNT_FLOW SPMC |current_flow, tick
________________________________________________________________________________________________________________________________________________________________
|RCP_DATA SPMC          |IN_RCP_DATA    |HEAT_ACT_TSK  |RT             |OUT_HTR_STATE      |HTR_STATE SPMC  |heater on/off, tick
|CRRNT_TEMP SPMC        |IN_CRNT_TEMP   |-//-//-//     |               |                   |                |
________________________________________________________________________________________________________________________________________________________________
|RCP_DATA SPMC          |IN_RCP_DATA    |PUMP_ACT_TSK  |RT             |OUT_PMP_STATE      |PMP_STATE SPMC  |pump on/off/rest, tick
|CRRNT_TEMP SPMC        |IN_CRNT_TEMP   |-//-//-//     |               |                   |                |
________________________________________________________________________________________________________________________________________________________________
|—                      |—              |RTC_TSK       |NON_RT         |OUT_RTC            |RT_TIME buffer  |C_TIME, tick
________________________________________________________________________________________________________________________________________________________________
