 ...
 *  - try_read() returns false before the first publish(), when publication
 *    changes during the claim window, or when per-slot sequence verification
 *    detects an overlapping slot write (single-shot: no internal retry).
 *  - try_read() returning true means out contains a stable, non-torn snapshot.
 *    Under concurrent publication it is not guaranteed to be the newest value
 ...
 *
 * TORN READ ACCEPTANCE EXCLUSION:
 *  - busy_mask/refcnt guide writer slot selection and reduce slot recycling
 *    pressure, but they are not the sole torn-read safety barrier.
 *  - Under concurrent publication/recycling windows, writer and reader may
 *    overlap on the same slot.
 *  - Such overlap is allowed by the protocol but must never be accepted as a
 *    successful read. The authoritative barrier is the per-slot sequence
 *    counter: reader accepts only if the same even seq value is observed before
 *    and after the payload copy.
 *
 * CLAIM / RELEASE COORDINATION:
 *  - Readers set busy_mask and increment refcnt before attempting a slot copy.
 *  - Readers decrement refcnt before the last counted reader may clear busy_mask.
 *  - busy_mask is a writer selection hint/coordination mask, not a strict proof
 *    that writer-reader slot overlap is impossible.
 *
 * PROGRESS:
 ...
 *
 * SPEC: primitives/docs/SPMCSnapshotSmp - RT Contract & Invariants.md (Rev 1.2)
 */
...
// Precondition: v != 0.
template <class UInt> SYS_FORCEINLINE uint32_t ctz_mask_smp(UInt v) noexcept
{
...
#ifdef STAM_TEST
template <typename T, uint32_t N> class SPMCSnapshotSmpTest;
#endif
...
template <typename T, uint32_t N> class SPMCSnapshotSmpCore final
{
  public:
    ...
    static_assert(N >= 1, "SPMCSnapshotSmp requires at least 1 reader (N >= 1)");
    static_assert(K <= busy_mask_bits, "SPMCSnapshotSmp: K = N+2 must fit in busy_mask word "
                                       "(N <= signal_mask_width - 2)");
    static_assert(N <= 254, "SPMCSnapshotSmp: N must fit in uint8_t refcnt (N <= 254)");
    static_assert(std::is_trivially_copyable_v<T>, "SPMCSnapshotSmp requires trivially copyable T");
  ...
  private:
    ...
    //
    // Authoritative torn-read acceptance barrier:
    // busy_mask/refcnt guide writer slot selection, but concurrent recycling can
    // still overlap a writer with a reader that selected the slot earlier.
    // A per-slot seqlock-style counter makes such overlap detectable: reader
    // accepts only when the same even seq value surrounds the payload copy.
    ...
    static_assert(sizeof(Seq) == SYS_CACHELINE_BYTES, "Seq must be exactly one cacheline");
    ...
    //                 Coordination mask for writer slot selection; it is not
    //                 the sole proof that reader↔writer overlap is impossible.
    ...
    //
    // Coordination with busy_mask (I5):
    //   readers set busy_mask and increment refcnt before copying;
    //   readers decrement refcnt before the last counted reader may clear
    //   busy_mask. This guides writer slot selection; seq verification is the
    //   final no-torn acceptance barrier.
    ...
    //
    // Slot selection (I3):
    //   Step 1: load busy_mask (acquire) — observes reader claims represented
    //           in the coordination mask.
    //   Step 2: load published (acquire) — current published slot index.
    //   Step 3: candidates = ~busy & ~(1<<pub) & all_mask.
    //           Slots neither busy in the observed mask nor published.
    //           By the Slot Availability Theorem, candidates != 0 for K=N+2.
    //   Step 4: j = ctz(candidates) — lowest-index free non-published slot.
    //
    // Write + publish:
    //   Step 5: slots[j] = value. j != pub is guaranteed by candidate selection.
    //           If a rare recycling race overlaps an older reader of j, that
    //           reader rejects the copy via seq verification.
    ...
    //
    // Returns false → no data yet (before first publish), publication changed
    //                 between claim and re-verify, or seq verification detected
    //                 an overlapping slot write (single-shot miss).
    // Returns true  → out contains a stable, non-torn snapshot. Under concurrent
    //                 publication it may be stale relative to method return time.
    //
    ...
    //   Step 3: SET claim:
    //             busy_mask |= (1<<i)  acq_rel  ← writer selection hint
    //             refcnt[i] += 1       acq_rel  ← counted reader claim
    //   Step 4: re-verify published (acquire). If changed: release and return false.
    //   Step 5: verify seq, copy slots[i].value, and verify seq again.
    //           This is the authoritative torn-read acceptance barrier.
    ...
    [[nodiscard]] bool try_read(T &out) noexcept
    {
        ...
        // Step 3: set claim. busy_mask guides writer selection; refcnt tracks
        // counted readers for release. Seq verification below is the final
        // no-torn acceptance barrier.
...
template <typename T, uint32_t N> class SPMCSnapshotSmpWriter final
{
  public:
    ...
    explicit SPMCSnapshotSmpWriter(SPMCSnapshotSmpCore<T, N> &core) noexcept : core_(core) {}
...
template <typename T, uint32_t N> class SPMCSnapshotSmpReader final
{
  public:
    static constexpr uint32_t K = SPMCSnapshotSmpCore<T, N>::K;
    using busy_mask_word_t = typename SPMCSnapshotSmpCore<T, N>::busy_mask_word_t;

    explicit SPMCSnapshotSmpReader(SPMCSnapshotSmpCore<T, N> &core) noexcept : core_(core) {}
    ...
    // Try to read a stable published snapshot (wait-free per invocation, O(1)).
    //
    // Returns false → no data yet, publication changed during the claim window,
    //                 or seq verification detected an overlapping slot write.
    // Returns true  → out contains a stable, non-torn snapshot.
...
template <typename T, uint32_t N> class SPMCSnapshotSmp final
{
  public:
