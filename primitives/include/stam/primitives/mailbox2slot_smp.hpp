...
template <typename T> class Mailbox2SlotSmpCore final
{
  public:
    static_assert(std::is_trivially_copyable_v<T>, "Mailbox2SlotSmp requires trivially copyable T");
...
#ifdef STAM_TEST
    friend class Mailbox2SlotSmpTest<T>;
#endif
  ...
  private:
    ...
    void publish(const T &value) noexcept
    {
        ...
        const uint8_t pub = ctrl.published.load(std::memory_order_relaxed);
        const uint8_t j = pub ^ 1u;

        // Step 2: open write window on slot j (seq → odd).
        seqs[j].seq.fetch_add(1u, std::memory_order_release);
        ...
        ctrl.has_value.store(true, std::memory_order_release);
    }
    ...
    [[nodiscard]] bool try_read(T &out) noexcept
    {
        ...
        if (!ctrl.has_value.load(std::memory_order_acquire))
        {
            return false;
        }
        ...
        const uint32_t s1 = seqs[i].seq.load(std::memory_order_acquire);
        if (s1 & 1u)
        {
        ...
        const uint32_t s2 = seqs[i].seq.load(std::memory_order_acquire);
        if (s1 != s2)
        {
...
template <typename T> class Mailbox2SlotSmpWriter final
{
  public:
    explicit Mailbox2SlotSmpWriter(Mailbox2SlotSmpCore<T> &core) noexcept : core_(core) {}
...
template <typename T> class Mailbox2SlotSmpReader final
{
  public:
    explicit Mailbox2SlotSmpReader(Mailbox2SlotSmpCore<T> &core) noexcept : core_(core) {}
...
template <typename T> class Mailbox2SlotSmp final
{
  public:
    friend class Mailbox2SlotSmpWriter<T>;
    friend class Mailbox2SlotSmpReader<T>;
