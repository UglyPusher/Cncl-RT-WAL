...
template <typename T, size_t Capacity> class SPSCRingDropOldestWriter;
template <typename T, size_t Capacity> class SPSCRingDropOldestReader;
#ifdef STAM_TEST
...
template <typename T, size_t Capacity> class SPSCRingDropOldestCore final
{
  public:
  ...
  private:
    ...
    [[nodiscard]] bool push(const T &item) noexcept
    {
        ...
        if (full)
        {
            ...
            dropped = tail_.compare_exchange_strong(expected, next_tail, std::memory_order_release,
...
template <typename T, size_t Capacity> class SPSCRingDropOldestWriter final
{
  public:
    ...
    // Returns true  → enqueued without dropping.
    // Returns false → exactly one oldest element was dropped to make room.
    [[nodiscard]] bool push(const T &item) noexcept { return core_.push(item); }
...
template <typename T, size_t Capacity> class SPSCRingDropOldestReader final
{
  public:
    ...
    // Pop an item from the ring (wait-free, bounded).
    // Returns true on success, false if the ring is empty.
    [[nodiscard]] bool pop(T &item) noexcept { return core_.pop(item); }
...
template <typename T, size_t Capacity> class SPSCRingDropOldest final
{
  public:
