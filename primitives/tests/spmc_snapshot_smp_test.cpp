...
template <typename T, uint32_t N> class SPMCSnapshotSmpTest final
{
  public:
    ...
    using SnapshotSmp = SPMCSnapshotSmp<T, N>;

    static busy_mask_word_t busy_mask(const SnapshotSmp &SnpSht) noexcept
    {
        return SnpSht.core_.ctrl.busy_mask.load(std::memory_order_relaxed);
    }

    static uint8_t refcnt_value(const SnapshotSmp &SnpSht, uint32_t i) noexcept
    {
...
TEST(test_stress_single_shot_miss_rate)
{
    ...
    const double miss_per_attempt =
        (attempts > 0) ? static_cast<double>(miss_count) / static_cast<double>(attempts) : 0.0;
    std::printf("    miss/read: %d/%d (miss/attempt=%.6f)\n", miss_count, read_count,
                miss_per_attempt);
    EXPECT(attempts > 0);
