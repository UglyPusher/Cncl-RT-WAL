...
template <typename T, uint32_t N> class SPMCSnapshotTest final
{
  public:
    ...
    using SPMCSnpsht = SPMCSnapshot<T, N>;

    static busy_mask_word_t busy_mask(const SPMCSnpsht &snpsht) noexcept
    {
        return snpsht.core_.ctrl.busy_mask.load(std::memory_order_relaxed);
    }

    static bool initialized(const SPMCSnpsht &snpsht) noexcept
    {
        return snpsht.core_.ctrl.initialized.load(std::memory_order_relaxed);
    }

    static uint8_t refcnt_value(const SPMCSnpsht &snpsht, uint32_t i) noexcept
    {
        return snpsht.core_.refcnt[i].load(std::memory_order_relaxed);
    }

    static constexpr uint32_t k_slots() noexcept { return Core::K; }

    static const char *slot_addr(const SPMCSnpsht &snpsht, uint32_t i) noexcept
    {
        return reinterpret_cast<const char *>(&snpsht.core_.slots[i]);
    }

    static const char *ctrl_addr(const SPMCSnpsht &snpsht) noexcept
    {
        return reinterpret_cast<const char *>(&snpsht.core_.ctrl);
    }
};
...
TEST(test_core_initial_state)
{
    ...
    using Test = SPMCSnapshotTest<Pod32, 2>;
    EXPECT(Test::busy_mask(ch) == 0u);
    EXPECT(Test::initialized(ch) == false);
    for (uint32_t i = 0; i < Test::k_slots(); ++i)
    {
        EXPECT(Test::refcnt_value(ch, i) == 0u);
    }
}
...
TEST(test_initialized_flag)
{
    ...
    EXPECT(Test::initialized(ch) == false);
    writer.write({7, 8});
    EXPECT(Test::initialized(ch) == true);
...
{
    SPMCSnapshot<Pod32, 2> ch;
    const bool aborted = stam::tests::expect_double_issue_abort([&] { (void)ch.writer(); });
    EXPECT(aborted);
}
...
{
    SPMCSnapshot<Pod32, 2> ch;
    const bool aborted = stam::tests::expect_issue_limit_abort(2, [&] { (void)ch.reader(); });
    EXPECT(aborted);
}
...
TEST(test_slot_cacheline_alignment)
{
    ...
    for (uint32_t i = 0; i < Test::k_slots(); ++i)
    {
        const auto addr = reinterpret_cast<uintptr_t>(Test::slot_addr(ch, i));
...
TEST(test_ctrl_cacheline_alignment)
{
    ...
    SPMCSnapshot<Pod32, 2> ch;
    using Test = SPMCSnapshotTest<Pod32, 2>;
    const auto addr = reinterpret_cast<uintptr_t>(Test::ctrl_addr(ch));
    EXPECT(addr % SYS_CACHELINE_BYTES == 0u);
}
...
TEST(test_ctrl_separate_from_slots)
{
    ...
    SPMCSnapshot<Pod32, 2> ch;
    using Test = SPMCSnapshotTest<Pod32, 2>;
    const auto ctrl_addr = reinterpret_cast<uintptr_t>(Test::ctrl_addr(ch));
    for (uint32_t i = 0; i < Test::k_slots(); ++i)
    {
        const auto slot_addr = reinterpret_cast<uintptr_t>(Test::slot_addr(ch, i));
        const auto diff = static_cast<ptrdiff_t>(ctrl_addr) - static_cast<ptrdiff_t>(slot_addr);
