...
public:
    ...
    // Привязка канального handle к порту payload — вызывается registry.assign_port()
    // через bind_port_fn в TaskWrapperRef. Делегирует в payload.bind_port() через if constexpr.
    // Если payload не имеет bind_port() — данный метод не существует;
    //   bind_port_fn в TaskWrapperRef = nullptr для таких payload.
    // Не предназначен для прямого вызова разработчиком.
    BindResult bind_port(PortName name, TypeErasedHandle handle) noexcept;
    //   (присутствует только если Payload реализует bind_port)
...
- `bind_port()` существует только если payload реализует `bind_port(PortName, TypeErasedHandle)`.
  Если метод отсутствует — `bind_port_fn` в `TaskWrapperRef` = `nullptr`; вызов через `assign_port()`
  возвращает `BindResult::payload_has_no_ports`.
- `bind_port()` вызывается только в bootstrap-фазе (до `seal()`), не в RT.
