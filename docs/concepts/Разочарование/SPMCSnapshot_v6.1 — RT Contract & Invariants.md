...
`docs/contracts/SPMCSnapshot — RT Contract & Invariants.md` · Revision 6.1 — February 2026
...
Оптимизирован для систем с ограниченной памятью: требует `(N+2) × sizeof(T)` памяти на данные. Writer и все reader'а — **wait-free, O(1)**. Torn read исключён структурно — без verify, без retry.
